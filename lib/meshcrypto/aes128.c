/*
 * aes128.c - AES-128 ECB, adapted from the public-domain tiny-AES-c (Unlicense).
 * https://github.com/kokke/tiny-AES-c  -- trimmed to AES-128 encrypt/decrypt.
 */
#include "aes128.h"
...
static void MixColumns(state_t *state)
{
    for (uint8_t i = 0; i < 4; ++i) {
        uint8_t t = (*state)[i][0];
        uint8_t tmp = (uint8_t)((*state)[i][0] ^ (*state)[i][1] ^ (*state)[i][2] ^ (*state)[i][3]);
        uint8_t tm;
...
void aes128_encrypt_block(const aes128_ctx_t *ctx, uint8_t block[16])
{
    ...
    AddRoundKey(Nr, state, ctx->round_key);
}
